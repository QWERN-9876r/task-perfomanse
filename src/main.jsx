...
worker.postMessage({items: TABS.all.items, onSize: onSize })

const List = ({type}) => {
    const [items, setItems] = React.useState({data: null})
    if ( type === 'all' ) worker.onmessage(setItems)

    return type === 'all' ? items.data
    : <ul className="section__panel-list">
        {TABS[type].items.map((item, index) =>
            <Event
                key={index}
                {...item}
                onSize={true}
    ...
    return <main className="main">
        ...
        <section className="section main__devices">
            ...
            <div className="section__panel-wrapper" ref={ref}>
                {TABS_KEYS.map(key =>
                    <div key={key} role="tabpanel" className={'section__panel' + (key === activeTab ? '' : ' section__panel_hidden')} id={`panel_${key}`} aria-labelledby={`tab_${key}`}>
                        { key === activeTab && <List type={key}/>}
                    </div>
                )}
                {activeTab === 'all' &&
